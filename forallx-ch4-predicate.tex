...
A definíció szerint a \define{QL kifejezés} vagy állandót, vagy változót jelent.

An \define{atomic formula of QL} is an n-place predicate followed by $n$ terms.

A \define{QL atomi formula} egy n-hely predátum, amelyet $n$ kifejezés követ.
...
Csakúgy, mint az SL esetében, a \emph{relatív definíciót} adunk a wff-nek QL-re nézve.
Valójában a meghatározás nagy része úgy fog kinézni, mint egy SL wff meghatározása: Minden
Az atomi formula egy wff, és a szentencia alkalmazásával új wff-eket építhet fel összekapcsolódva
...
Csak hozzáadhatunk egy szabályt az egyes mennyiségi meghatározókhoz, és meg lehet vele csinálni . Például: Ha \script{A} egy wff, akkor $\forall x\script{A}$ és $\exists x\script{A}$ is wff. Ez azonban meg kéne , hogy engedjen olyan furcsa mondatokat, mint  $\forall x\exists x Dx$ és $\forall x Dw$. Mit is jelenthet ez? Megismerhetnénk az ilyen mondatok értelmezését, ám ehelyett mi a wff definícióját írjuk, úgy, hogy az ilyen furcsa dolgok még nem is formáltak
...
Annak érdekében, hogy $\forall x\script{A}$ wff legyen, az \script{A}-nak tartalmaznia kell az $x$ változót, és nem tartalmazhat már x-számlálót. Az $\forall x Dw$ nem számít wff-ként, mivel az `$x$' nem fordul elő $Dw$-ben, és $\forall x \exists x Dx$ nem számít wff-ként, mivel az ∃xDx tartalmaz x-számlálót
...
\item Ha \script{A} egy wff, \script{x} egy változó, \script{A} legalább egy \script{x} előfordulását tartalmazza, és az \script{A} nem tartalmaz \script{x}-számlálót, akkor az $\forall\script{x}\script{A}$ egy wff.
...
\item Ha \script{A} egy wff, \script{x} egy változó, \script{A} legalább egy \script{x} előfordulását tartalmazza, és az \script{A} nem tartalmaz \script{x}-számlálót, akkor az $\exists\script{x}\script{A}$ egy wff.
...
Vegye figyelembe, hogy a fenti definícióban megjelenő '\script{x}' nem az $x$ változó. A \emph{metaváltozó} jelenti a QL bármely változóját. Tehát az $\forall xAx$ egy wff, de ugyanúgy, mint a $\forall yAy$, $\forall zAz$, $\forall x_4Ax_4$, and $\forall z_9Az_9$.
...
Most megadhatjuk a hatókör hivatalos meghatározását: A számszerűsítő hatóköre az al-képlet, amelynek a számszerűsítője a fő logikai operátor.
